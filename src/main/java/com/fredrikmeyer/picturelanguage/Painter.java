...
public class Painter {
    ...
    public Painter(Frame frame, File file) throws IOException {
        this(frame);
        BufferedImage imageFromFile = cropImage(ImageIO.read(file));

        graphics = imageFromFile.createGraphics();


        //this.bufferedImage = new BufferedImage(imageFromFile.getHeight(),
//                imageFromFile.getWidth(), imageFromFile.getType());
        //      AffineTransform af = affineTransformationFromFrame(frame);
//        AffineTransformOp op = new AffineTransformOp(af, AffineTransformOp.TYPE_BILINEAR);
        //      op.filter(imageFromFile, this.bufferedImage);
    }
    ...
    public AffineTransform affineTransformationFromFrame(Frame frame) {
        ...
        transformation.concatenate(tform);
